...
public class ChineseCalendar {
     ...
     * @throws Exception
     */
    public static String lunarToSolar(String lunarDate, boolean flag) throws Exception{
        ...
        checkLunarDate(lunarYear, lunarMonth, lunarDay, flag);
        ...
        if(flag & leapMonth != lunarMonth){
            throw(new Exception("您输入的闰月标志有误！"));
        }

        //当年没有闰月或月份早于闰月或和闰月同名的月份
        if(leapMonth==0|| (lunarMonth < leapMonth) || (lunarMonth==leapMonth && !flag)){
     ...
     * @throws Exception
     */
    public static String solarToLunar(Date solarDate, Boolean isFormat) throws Exception{
        int i;
        int temp = 0;
        int lunarYear;
        int lunarMonth; //农历月份
